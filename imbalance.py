# -*- coding: utf-8 -*-
"""imbalance.ipynb のコピー

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1thBTG8w4QdPCWU_CVeGVNFKafU5K-oOr
"""
...
def main():
    ...
    # wordとword_cntを作成
    ans_list = []
    for idx in range(len(df_trainval)):
        answers = [answer2idx[process_text(answer["answer"])] for answer in df_trainval["answers"][idx]]
        mode_answer_idx = mode(answers)
        ans_list.append(idx2answer[mode_answer_idx])
    df_trainval["word"] = ans_list
    df_trainval["word_cnt"] = df_trainval["word"].map(df_trainval["word"].value_counts())
    df_trainval["is_unanswerable"] = np.array(df_trainval["word"]=="unanswerable").astype(int)
    # 訓練データのunanswerableをアンダーサンプリング
    df_unans = df_trainval[df_trainval["is_unanswerable"] == 1].reset_index(drop=True)
    df_not_unans = df_trainval[df_trainval["is_unanswerable"] == 0].reset_index(drop=True)
    kf = KFold(n_splits=2, shuffle=True, random_state=42) # df_unansを5:5で分割→2通りの訓練データ
    cnt = 0
    for tr_idx, va_idx in kf.split(df_unans):
        df_unanswerable = df_unans.iloc[tr_idx].reset_index(drop=True) # アンダーサンプリングしたunanswerable
        df_train = pd.concat([df_unanswerable, df_not_unans], axis=0) # 結合後の訓練データ
        df_train = df_train.sample(frac=1, random_state=42).reset_index(drop=True)
        df_train = df_train.drop(columns=["word", "word_cnt", "is_unanswerable"])
        # データセットの作成
        ...
        for epoch in range(num_epoch):
            ...
            if epoch+1==10:
                ...
                np.save("drive/MyDrive/Colab Notebooks/DLBasics2024_colab/final/ensemble/pred_submission_10epochs_"+str(cnt+4)+".npy", submission)
                # 確率値ファイルの作成
                submission_prob = np.array(submission_prob)
                np.save("drive/MyDrive/Colab Notebooks/DLBasics2024_colab/final/ensemble/prob_submission_10epochs_"+str(cnt+4)+".npy", submission_prob)
        cnt += 1
